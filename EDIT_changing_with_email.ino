...
char ssid[] = "LS-5002";    //  your network SSID (name)
char pass[] = "murthyjio1234";   // your network password
...
//SD CARD-----------------------------------------------
  ...
  myFile = SD.open("TrialRun.csv");
  if (myFile) {
    Serial.println("TrialRun.csv:");
    // read from the file until there's nothing else in it:
    Serial.write("PRINTING TRIALRUN");
...
Serial.println("PRINTING STARTS HERE");
...
Serial.println("INTERNET STATUS : CONNECTED");
 ...
 if (!SD.begin(7)) 
  ...
  count=0;
  //myFile.print("initialization done.SD Card reinserted");
   }
   //Serial.println("hai");
   myFile.print(",");
  myFile.print(num);
  num++;
   ...
   while(temperature1>=70 || temperature2>=70 || temperature3>=70 || temperature4>=70 || temperature5>=70 || temperature6>=70 || temperature7>=70)
   {
    tone(buzzer, 400);
    break;
   }
   ...
   if(temperature1>=70)
   {
    ...
    {
      len = loadBuffer(temperature1,temperature2,temperature3,temperature4,temperature5,temperature6,temperature7,humid,state_door1,state_door2,state_door3);  
    sendEmail();
   }}
   if(temperature2>=70)
   {
...
int loadBuffer(int temp,int temp1,int temp2,int temp3,int temp4,int temp5,int temp6,int humidity,int door1_state,int door2_state,int door3_state) 
{ 
